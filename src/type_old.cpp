...
static TypeDefn *get_type_defn(Module *module, AstType *type)
{
    ...
    else
    {
        ...
        for (i64 i = 0; i < module->type_defns_count; ++i)
        {
            ...
            if (type->args.count != defn->args.count)
                continue;

            if ((type->ret == NULL) != (defn->ret == NULL))
                continue;

            if (type->ret)
            {
                if (get_type_defn(module, type->ret) != defn->ret)
                    continue;
            }

            bool match = true;
            for (i64 j = 0; j < type->args.count; ++j)
            {
                if (get_type_defn(module, type->args[j]) != defn->args[j])
                {
                    match = false;
                    break;
                }
        ...
        // No match, so make a new type definition.
        assert(module->type_defns_count < (sizeof(module->type_defns) / sizeof(module->type_defns[0])));
        TypeDefn *defn = &module->type_defns[module->type_defns_count++];
        defn->name = NULL;
        defn->size = 8; // 64-bit pointer size.
        ...
        for (auto &arg : type->args)
            defn->args.add(get_type_defn(module, arg));
        defn->ret = get_type_defn(module, type->ret);
...
static TypeDefn *determine_expr_type(Module *module, AstExpr *expr)
{
    switch (expr->type)
    {
        case AST_EXPR_IDENT:
        {
            ...
            if (var)
                return var->name->type_defn;

            // Check for function pointers.
            auto func = module_get_func(module, ident);
            if (func)
                return func->type_defn;
